...
Access the web interface by navigating to http://<your-server-ip>:8000 in your browser.

Initial Setup

Create the Admin Account: The very first user to register an account on the web interface is automatically granted Administrator privileges.

Lock Registrations: Once logged in, go to the "Admin" tab and toggle Open Registration to "CLOSED" to prevent unauthorized users from using your server.

Generate a Token: You will use the login credentials (specifically the Bearer token managed under the hood) to connect your Obsidian vault.
...
Enter your Server API URL (e.g., http://192.168.1.100:8000).

Enter your Server API Token (copied from the web dashboard network requests, or managed via the plugin auth flow).

Input your Gemini API Key and validate it.

Select your preferred LLM model.

Point the plugin to a local .md file in your vault to act as the prompt template (Available variables: {title}, {url}, {content}, {vault_context}).

Click "Sync" to push your preferences and vault context to the server.
